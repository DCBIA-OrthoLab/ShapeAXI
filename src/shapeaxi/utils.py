...
def GetNeighborsT(cells, pid):
    """
    cells: Tensor of shape (M, K) where M is the number of cells and K is the number of points per cell.
    pid: Integer, the point index for which we want to find neighbors.
    """
    # Find all cells containing the point `pid`
    cell_mask = (cells == pid).any(dim=1)
    
    # Get the list of all cells that contain the point `pid`
    relevant_cells = cells[cell_mask]
    
    # Collect all points from the relevant cells (except `pid`)
    neighbors = relevant_cells[relevant_cells != pid]
    
    # Return the unique neighboring points
...
def UnitVerts(verts):
    ...
    verts = verts - mean_v
    scale_factor = 1.0/torch.linalg.vector_norm(max_verts - mean_v)
