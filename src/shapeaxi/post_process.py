...
def MedianFilter(vtkdata, labels, label=None):

	labels_np = vtk_to_numpy(labels).copy()

	for pid in range(len(labels_np)):
		
		if label is None or labels_np[pid] == label:

			neighbors_pid = GetNeighbors(vtkdata, pid)

			median = np.median(labels_np[neighbors_pid])

			labels.SetTuple(int(pid), (median,))
