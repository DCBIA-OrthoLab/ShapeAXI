...
#         #####Left path
#         efficient_netL = torchvision.models.resnet18()
#         efficient_netL.conv1 = nn.Conv2d(self.hparams.nbr_features, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
#         elif self.hparams.layer == 'IcoConv2D':
#             out_size = 256
...
#             self.poolingL = AvgPoolImages(nbr_images=self.nbr_cam)
#         elif self.hparams.layer == 'IcoLinear':
#             out_size = 256
...
#         #####Right path
#         efficient_netR = torchvision.models.resnet18()
#         efficient_netR.conv1 = nn.Conv2d(self.hparams.nbr_features, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
class SaxiIcoClassification(pl.LightningModule):
    ...
    def __init__(self, **kwargs):
        ...
        # model_params = eval('dict(%s)' % self.hparams.base_encoder_params.replace('',''))
        # self.convnet = template_model(**model_params)
        # self.TimeDistributedL = TimeDistributed(self.convnet)
        ...
        # model_params = eval('dict(%s)' % self.hparams.base_encoder_params.replace('',''))
        # self.convnet = template_model(**model_params)
        # self.TimeDistributedR = TimeDistributed(self.convnet)
        # output_size = model_params.get('num_classes',None)
        ...
        out_size = 256
        # Left path
        self.create_path('L', out_size)
        # Right path
        self.create_path('R', out_size)
        #Demographics
        self.normalize = nn.BatchNorm1d(self.hparams.nbr_demographic)
        #Final layer
        self.Classification = nn.Linear(2*out_size+self.hparams.nbr_demographic, 2)
    ...
    def create_path(self, side, out_size):
        if hasattr(monai.networks.nets, self.hparams.base_encoder):
            template_model = getattr(monai.networks.nets, self.hparams.base_encoder)
            print('Monai network')
        elif hasattr(torchvision.models, self.hparams.base_encoder):
